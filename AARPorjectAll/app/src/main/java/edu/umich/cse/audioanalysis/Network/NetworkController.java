...
public class NetworkController {
	private final static String DEFAULT_SERVER_ADDR = C.SERVER_ADDR;
	private final static int DEFAULT_SERVER_PORT = C.DETECTER_SERVER_PORT;
	...
	public NetworkController(NetworkControllerListener callerIn) {
		caller = callerIn;
	...
	private void keepPacketRecving(){
		while(keepRecvThreadRunning) {
			try {
				...
				Log.d(C.LOG_TAG, "recv from socket =" + f + ", byteIsRead =" + byteIsRead);
				...
				//e.printStackTrace();
				Log.w(C.LOG_TAG, "[WARN]: read inputstream timeout (need more data? or wait a while?)");
			}
		}
	}
