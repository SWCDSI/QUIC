...
public class GaugeView extends View {
	...
	// yctung: hack to use constant value
	private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		mShowOuterShadow = true;// a.getBoolean(R.styleable.GaugeView_showOuterShadow, SHOW_OUTER_SHADOW);
		mShowOuterBorder = true;// a.getBoolean(R.styleable.GaugeView_showOuterBorder, SHOW_OUTER_BORDER);
		mShowOuterRim = true;// a.getBoolean(R.styleable.GaugeView_showOuterRim, SHOW_OUTER_RIM);
		mShowInnerRim = true;// a.getBoolean(R.styleable.GaugeView_showInnerRim, SHOW_INNER_RIM);
		mShowNeedle = true;// a.getBoolean(R.styleable.GaugeView_showNeedle, SHOW_NEEDLE);
		mShowScale = true;// a.getBoolean(R.styleable.GaugeView_showScale, SHOW_SCALE);
		mShowRanges = true;// a.getBoolean(R.styleable.GaugeView_showRanges, SHOW_RANGES);
		mShowText = false;// a.getBoolean(R.styleable.GaugeView_showGaugeText, SHOW_TEXT);

		mOuterShadowWidth = OUTER_SHADOW_WIDTH; //mShowOuterShadow ? a.getFloat(R.styleable.GaugeView_outerShadowWidth, OUTER_SHADOW_WIDTH) : 0.0f;
		mOuterBorderWidth = OUTER_BORDER_WIDTH; //mShowOuterBorder ? a.getFloat(R.styleable.GaugeView_outerBorderWidth, OUTER_BORDER_WIDTH) : 0.0f;
		mOuterRimWidth = OUTER_RIM_WIDTH; //mShowOuterRim ? a.getFloat(R.styleable.GaugeView_outerRimWidth, OUTER_RIM_WIDTH) : 0.0f;
		mInnerRimWidth = INNER_RIM_WIDTH; //mShowInnerRim ? a.getFloat(R.styleable.GaugeView_innerRimWidth, INNER_RIM_WIDTH) : 0.0f;
		mInnerRimBorderWidth = INNER_RIM_BORDER_WIDTH; // mShowInnerRim ? a.getFloat(R.styleable.GaugeView_innerRimBorderWidth, INNER_RIM_BORDER_WIDTH) : 0.0f;

		mNeedleWidth = NEEDLE_WIDTH; // a.getFloat(R.styleable.GaugeView_needleWidth, NEEDLE_WIDTH);
		mNeedleHeight = NEEDLE_HEIGHT; // a.getFloat(R.styleable.GaugeView_needleHeight, NEEDLE_HEIGHT);

		mScalePosition = SCALE_POSITION; // (mShowScale || mShowRanges) ? a.getFloat(R.styleable.GaugeView_scalePosition, SCALE_POSITION) : 0.0f;
		mScaleStartValue = SCALE_START_VALUE; // a.getFloat(R.styleable.GaugeView_scaleStartValue, SCALE_START_VALUE);
		mScaleEndValue = SCALE_END_VALUE; // a.getFloat(R.styleable.GaugeView_scaleEndValue, SCALE_END_VALUE);
		mScaleStartAngle = SCALE_START_ANGLE; // a.getFloat(R.styleable.GaugeView_scaleStartAngle, SCALE_START_ANGLE);
        mScaleEndAngle = 360.0f-mScaleStartAngle; //a.getFloat(R.styleable.GaugeView_scaleEndAngle, 360.0f - mScaleStartAngle);

		mDivisions = SCALE_DIVISIONS;// a.getInteger(R.styleable.GaugeView_divisions, SCALE_DIVISIONS);
		mSubdivisions = SCALE_SUBDIVISIONS;// a.getInteger(R.styleable.GaugeView_subdivisions, SCALE_SUBDIVISIONS);

		if (mShowRanges) {
			mTextShadowColor = Color.RED; //a.getColor(R.styleable.GaugeView_textShadowColor, TEXT_SHADOW_COLOR);

            final CharSequence[] rangeValues = {"16","25","40","100"};//a.getTextArray(R.styleable.GaugeView_rangeValues);
            final CharSequence[] rangeColors = { "#1BCA21" , "#E8E721", "#E86F21" , "#E7202B"}; //a.getTextArray(R.styleable.GaugeView_rangeColors);
